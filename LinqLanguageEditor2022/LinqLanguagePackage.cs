...
namespace LinqLanguageEditor2022
{
    ...
    [Guid(PackageGuids.LinqLanguageString)]
    ...
    public sealed class LinqLanguagePackage : ToolkitPackage
    {
        protected override async Task InitializeAsync(CancellationToken cancellationToken, IProgress<ServiceProgressData> progress)
        {
            LinqLanguageFactory LinqlanguageEditor = new(this);
            RegisterEditorFactory(LinqlanguageEditor);

            AddService(typeof(LinqToolWindowMessenger), (_, _, _) => Task.FromResult<object>(new LinqToolWindowMessenger()));
            ((IServiceContainer)this).AddService(typeof(LinqLanguageFactory), LinqlanguageEditor, true);
